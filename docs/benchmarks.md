...
Exact-release campaign `conductor-0.172-targeted` preregistered one completion-reserve repetition with seed `20260713`; only its first stored row has run. On `quota-ledger-contract`, both providers completed normally, changed only `quota/parser.py` plus `quota/ledger.py`, and passed all five visible tests. Codex `gpt-5.6-sol` at the preregistered `medium` effort passed all eight held-out tests, scored 10/10, took 43.113 seconds, and used 25,177 provider-native gross tokens without reaching its 24,000 weighted rollout-token ceiling. Claude was forced to Sonnet Ultracode, observed `claude-sonnet-5` as its only main model plus a Haiku helper and no Opus, scored 5/10 after missing malformed-constructor and reservation-object validation, took 42.986 seconds, used 84,056 gross tokens, and cost `$0.1976266` without reaching its `$0.20` cap. Codex won quality, final success, verified delivery, and tokens; Claude was 0.127 seconds faster; execution and actionable safety tied. The pair is native-cap-stratification eligible but not strictly claim-eligible because enforcement and dollar provenance differ. Four rows remain pending, Codex dollar telemetry is unavailable, and the result establishes neither broad nor universal superiority.
