...
- safe saved workflow commands under `.claude/workflows/` using either non-executed JSON-compatible Conductor exports or a statically compiled Claude-style `agent()`/`pipeline()`/`phase()`/static-`parallel()` subset with bounded map chaining and synthesis,
...
This compatibility path accepts static literals, top-level awaited `agent()` and `pipeline()` bindings, literal or `args.NAME` item arrays, prior-agent array schema properties, prior map results, callback template interpolation, static `schema`/`label` options, and exact `.filter(Boolean)` on a map binding, consuming map source, synthesis reference, or terminal return. It preserves statement order and duplicate list elements, emits one `agent_map` packet per array element, caps concurrency at 16 and the script's total possible agents at 1,000, and compiles every provider step read-only. A consumed schema is normalized for strict output, materialized under the external run, passed to `codex exec --output-schema`, and read back through a strict bounded JSON pointer. A prior map passes through a deterministic one-MiB internal `collect_results` artifact before a later map reads it: text/string-schema outputs use bounded opaque semantics, object-schema outputs use bounded JSON semantics, and every consumer directly depends on the collector. A later single `agent()` may instead interpolate that map directly or through exact `.filter(Boolean)`, `.join()` with one static delimiter, or `JSON.stringify()`; the compiler substitutes an explicit evidence marker and transfers the same collector through `context_from`. Repeated equivalent references reuse one artifact. The terminal return remains a separate `collect_results` step and `claude-workflow/result.json`, with a 10 MiB aggregate ceiling, JavaScript-compatible falsey filtering, byte-level source/output receipts, and no additional model call. The CLI reports the result artifact path by default to avoid context expansion; `--print-result` explicitly renders the completed value. Imports, loops, branches, mutation, dynamic delimiters, arbitrary other calls/transforms, nested callback fan-out, computed interpolation, unsupported options, and every other script construct fail closed.

Neither format evaluates JavaScript or requires Node. The loader rejects symlinked or workspace-escaping paths, uses component-wise no-follow reads and exports, rejects same-precedence duplicate names, validates the compiled workflow with the normal schema, and runs it through the same `WorkflowRunner` and `RuntimePolicy` gates as JSON workflows. `agent()`, `pipeline()`, `parallel()`, and `phase()` are compile-time syntax forms, not JavaScript calls performed by the loader. The static compiler accepts JSON-compatible constants, exact `args.NAME` and `args && args.NAME` aliases, bounded `+` text composition in prompts and labels, static `whenToUse` discovery metadata, static agent effort, declared phase metadata, phase markers/options, the common `parallel(SOURCE.map(item => () => agent(...)))` thunk form, bounded top-level map-to-map dataflow, and map-to-single-agent synthesis. Claude `max` effort compiles to Codex `ultra`; all other shared effort names are preserved. Static object maps are still pre-rendered into bounded opaque prompts. A dynamic or prior-result map whose callback reads `item.property` instead uses `item_semantics: "json"`: inline or JSON-pointer object arrays are canonicalized into one object per packet, each object is limited to 64 KiB, 16 nesting levels, and 512 JSON nodes, and prompt rendering performs local dotted-key lookup without expression evaluation. Missing fields, duplicate keys, non-finite numbers, malformed objects, secrets, NULs, marker collisions, source/result drift, or ambiguous packetization fail before the downstream provider launches. JSON packet labels expose only a SHA-256 prefix across outputs, cache, retry, traces, and dashboards. Dynamic string parallel sources and map-result text are bounded and opaque; ordinary pipeline string sources retain their existing workspace-path semantics. Internal collectors are permitted only for a prior `agent_map`, must feed at least one later direct-dependent map at the JSON root or one direct-dependent synthesis context, cannot alias the terminal result, and retain strict output/source collision checks. Synthesis contexts reopen and reconcile the collector immediately before launch, then use the existing redacted, marker-escaped, path-free 64-KiB evidence envelope. `validate-saved-workflows --keep-going` performs a complete non-executing corpus scan and returns nonzero after reporting every invalid script. Capability flags such as `--allow-agent`, `--allow-parallel`, `--allow-writes`, `--allow-network`, and `--approve` remain mandatory when the resulting workflow needs them. Arbitrary JavaScript, helper functions, branches/loops, nested callback fan-out, promise/arbitrary transforms beyond the exact rendering forms, richer multi-callback `pipeline()` forms, model/custom-agent/worktree options, write-capable Claude script compilation, and automatic background launch are not claimed by this subset.
...
`codex_exec.context_from` is the explicit artifact handoff contract. It accepts one through 32 unique direct `depends_on` step ids. Sources may be `write_artifact`, `shell` with `capture`, `codex_exec`, `agent_map`, or an intermediate `collect_results`; ordering a step with `depends_on` alone never injects artifact content. Validation rejects other targets, indirect sources, unsupported kinds, uncaptured shell sources, duplicates, unsafe ids, and over-limit lists. A map backed by a runtime file/artifact must declare `max_packets`, and the statically known combined artifact upper bound must fit 32 before any workflow process starts. An intermediate collector remains one artifact regardless of its bounded source-map width and must satisfy its own direct-consumer contract. Model-authored workflows use the same field and cross-step validation.
