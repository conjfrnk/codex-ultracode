...
- safe saved workflow commands under `.claude/workflows/` using either non-executed JSON-compatible Conductor exports or a statically compiled Claude-style `agent()`/`pipeline()` subset,
...
- full JavaScript saved workflow `args` semantics; Conductor supports only safe `{{args.KEY}}` placeholder rendering,
