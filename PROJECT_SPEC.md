...
### Bounded Claude map-to-synthesis amendment

The non-evaluating Claude compiler may let a later top-level `agent()` text template consume one or more prior map bindings. An interpolation may use the binding directly, exact `.filter(Boolean)`, exact `.join(STATIC_STRING)`, or exact `JSON.stringify(BINDING)`; the filter may appear inside the latter rendering forms. Static join delimiters are decoded with the existing string grammar and bounded by source and decoded length. Dynamic delimiters, dotted map properties, a map used as the complete non-text prompt, nested calls, other methods, callbacks, expressions, coercion, and JavaScript evaluation remain rejected.

Every filtered or unfiltered map view required by synthesis must reuse or create the same deterministic one-MiB intermediate `collect_results` artifact used by map-to-map handoff. Equivalent repeated references reuse one collector. The compiler replaces each interpolation with an explicit source/collector evidence marker and sets the collector as both `context_from` and a direct dependency of the synthesis `codex_exec`. A single synthesis may reference at most the existing 32 context sources, top-level order remains authoritative, and forward map references fail before workflow creation.

An intermediate collector may feed a later map at the strict JSON root, a later synthesis context, or both. It still must source a completed direct-dependent `agent_map`, have a unique output outside the source capture and terminal result, and be consumed by at least one later direct-dependent step. The separate non-intermediate terminal collector remains mandatory and final whenever `result_artifact` exists. Adding collector context support must not permit a terminal collector, arbitrary file, skipped step, indirect dependency, or unsupported step kind to become evidence.

Before rendering a collector into synthesis context, the runtime must recompute its ordered source material, falsey filtering, source hash/count, item count, canonical payload, output bytes, and output hash. The artifact is then reopened through the existing regular no-follow, size/hash-bound context reader. The existing path-free, secret-redacted, marker-escaped envelope remains capped at 32 artifacts, eight KiB per excerpt, and 64 KiB overall; state retains only its contract, counts, byte size, and hash. Source drift, collector drift, symlink substitution, receipt loss, overflow, malformed JSON, or a race before launch must fail without starting the synthesis provider.

Provider-free verification must cover direct, filtered, static-joined, and JSON-rendered references; equivalent-reference deduplication; distinct filtered/unfiltered views; structured and text map output; forward, dotted, non-text, dynamic-delimiter, arbitrary-transform, over-source, and malformed rendering rejection; collector-as-context validation; immediate pre-launch collector tampering; real map-to-synthesis execution and terminal collection; schema/inspection/dry-run behavior; the packaged saved-script corpus; and unchanged map-to-map, direct-agent, declarative, context, and result contracts. This amendment closes the common many-reviewers-to-one-summary shape but does not establish general JavaScript loops, conditions, helpers, transforms, nested fan-out, write-capable scripts, or broad/universal Claude/Ultracode superiority.

One separately approved exact-0.172 matched row provides targeted forward quality evidence without weakening those boundaries. Under the same completion-reserve quota contract, source, permissions, visible verifier, hidden evaluator, and 0.172 runtime, Codex `gpt-5.6-sol` medium and pinned Sonnet Ultracode both completed normally and changed only the two allowed modules. Codex passed 5 visible plus 8 held-out tests for 10/10 in 43.113 seconds with 25,177 gross tokens. Sonnet passed visible verification but missed constructor and reservation-object validation, scored 5/10 in 42.986 seconds, used 84,056 gross tokens, and cost `$0.1976266`; observed evidence names `claude-sonnet-5` as the only main model, one Haiku helper, and no Opus. The pair is native-cap-stratification eligible, not strictly claim-eligible, and cannot establish broad, universal, latency, or cost superiority.
